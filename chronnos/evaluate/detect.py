...
class CHRONNOSDetector:
    ...
    def __init__(self, model_path=None, model_name='chronnos_v1_0.pt', device=None):
        """
        Initializes a CHRONNOS model for detections. Specify either model_path or model_name. A local copy of 'model_name' is downloaded to the home directory.
    ...
    def predict(self, map_paths, reproject=False, num_workers=4):
        """
        Detect coronal holes for a set of observations. Use predict_dir to auto scan a directory.
    ...
    def ipredict(self, map_paths, reproject=False, num_workers=4):
        """
        Detect coronal holes for a set of observations. The results are returned as data stream.
        :param map_paths: paths of the FITS files. The format is (channel, file_path).
        ...
        with torch.no_grad():
            for batch, ref in tqdm(zip(map_loader, map_paths[3]), desc='model prediction'):
                batch = batch.to(self.device)
                pred = self.model(batch).detach().cpu() >= 0.5
